...
- 主出口和每个备用出口可分别填写服务商分配的 NAT 端口池；每条规则按出口独立分配中继端口
- 出口 Agent 自动检查“出口 → 落地”：全部规则上报 ICMP 延迟和丢包，TCP / TCP+UDP 规则额外检查真实业务端口的 TCP 握手状态；探测共享最多 8 个并发，不发送业务数据
- 可选自动故障切换：连续 3 次失败后同时切换出口内网 IP 和对应中继端口，主出口连续 3 次恢复后按优先级回切
...
各出口可用中继端口：每台 NAT 机器分别填写服务商分配的端口，例如主出口 1301-1349、备用出口 2301-2349；留空不限制
入口引擎：nftables / Realm（入口公网端口 → 出口内网 IP:中继端口）
出口引擎：nftables / Realm（出口中继端口 → 落地 IP:端口）
```

随后在这条线路中新增转发，填写入口公网端口和落地 IP/端口。入口 Agent 只执行入口引擎，出口 Agent 只执行出口引擎；两者互不绑定。面板会从每个出口自己的端口池分别分配中继端口，并提前把对应出口段下发到全部主备出口。发生切换时，入口目标会同时改为备用出口的内网 IP 和它自己的中继端口，无需人工修改规则。入口监听端口仍可使用 1-65535；若两个线路共享同一备用出口，面板会按该出口实际端口检查并避开冲突。
