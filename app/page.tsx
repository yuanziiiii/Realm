...
type Node = { id:string; name:string; role:NodeRole; public_address:string; private_address:string; public_interface:string; private_interface:string; default_relay_port_range:string; status:string; agent_version?:string; applied_revision?:number; apply_status?:string; apply_error?:string; last_seen_at:string };
type Line = { id:string; name:string; mode:Mode; ingress_node_id:string; egress_node_id:string; egress_node_ids:string[]; active_egress_node_id:string; failover_enabled:boolean; listen_address:string; relay_port_range:string; egress_port_ranges?:Record<string,string>; engine:Engine; ingress_engine:Engine; egress_engine:Engine; enabled:boolean };
type Rule = { id:string; line_id:string; mode:Mode; name:string; protocol:"tcp"|"udp"|"both"; ingress_node_id:string; egress_node_id:string; listen_address:string; listen_port:number; relay_port:number; relay_ports?:Record<string,number>; target_host:string; target_port:number; engine:Engine; ingress_engine:Engine; egress_engine:Engine; upload_mbps:number; download_mbps:number; burst_kbytes:number; enabled:boolean };
...
const demoNodes:Node[] = [
  {id:"in-gz",name:"广州入口",role:"ingress",public_address:"203.0.113.18",private_address:"10.24.0.2",public_interface:"eth0",private_interface:"wg0",default_relay_port_range:"",status:"online",agent_version:"0.3.6",apply_status:"normal",last_seen_at:new Date().toISOString()},
  {id:"out-hkg",name:"香港出口",role:"egress",public_address:"198.51.100.24",private_address:"10.24.0.3",public_interface:"eth0",private_interface:"wg0",default_relay_port_range:"32000-32999",status:"online",agent_version:"0.3.6",apply_status:"normal",last_seen_at:new Date().toISOString()},
  {id:"out-sg",name:"新加坡备用",role:"egress",public_address:"198.51.100.52",private_address:"10.24.0.4",public_interface:"ens3",private_interface:"wg0",default_relay_port_range:"42000-42999",status:"offline",last_seen_at:new Date(Date.now()-3600_000).toISOString()},
];
const demoLines:Line[] = [
  {id:"line-gz-hk",name:"广州 → 香港专线",mode:"dual_managed",ingress_node_id:"in-gz",egress_node_id:"out-hkg",egress_node_ids:["out-hkg","out-sg"],active_egress_node_id:"out-hkg",failover_enabled:true,listen_address:"0.0.0.0",relay_port_range:"32000-32999",egress_port_ranges:{"out-hkg":"32000-32999","out-sg":"42000-42999"},engine:"realm",ingress_engine:"nftables",egress_engine:"realm",enabled:true},
  {id:"line-hk-exit",name:"香港出口接管",mode:"exit_only",ingress_node_id:"out-hkg",egress_node_id:"out-hkg",egress_node_ids:["out-hkg"],active_egress_node_id:"out-hkg",failover_enabled:false,listen_address:"10.24.0.3",relay_port_range:"30000-39999",egress_port_ranges:{"out-hkg":"30000-39999"},engine:"realm",ingress_engine:"realm",egress_engine:"realm",enabled:true},
];
...
const blankNode={name:"",role:"ingress" as NodeRole,public_address:"",private_address:"",public_interface:"",private_interface:"",default_relay_port_range:""};
const blankLine={name:"",mode:"dual_managed" as Mode,ingress_node_id:"",egress_node_id:"",egress_node_ids:[] as string[],active_egress_node_id:"",failover_enabled:false,listen_address:"",relay_port_range:"",egress_port_ranges:{} as Record<string,string>,engine:"nftables" as Engine,ingress_engine:"nftables" as Engine,egress_engine:"nftables" as Engine,enabled:true};
...
function linePortRange(line:Pick<Line,"egress_node_id"|"relay_port_range"|"egress_port_ranges">,egressID:string){return Object.prototype.hasOwnProperty.call(line.egress_port_ranges||{},egressID)?line.egress_port_ranges?.[egressID]||"":line.relay_port_range||""}
function ruleRelayPort(rule:Pick<Rule,"egress_node_id"|"relay_port"|"relay_ports">,egressID:string){return rule.relay_ports?.[egressID]||rule.relay_port}
function moveOrdered<T extends {id:string}>(items:T[],id:string,direction:MoveDirection,within?:(item:T)=>boolean){const eligible=items.map((item,index)=>({item,index})).filter(({item})=>!within||within(item));const position=eligible.findIndex(({item})=>item.id===id),target=position+direction;if(position<0||target<0||target>=eligible.length)return items;const next=[...items],from=eligible[position].index,to=eligible[target].index;[next[from],next[to]]=[next[to],next[from]];return next}
...
export default function Home(){
  ...
  const [nodeModal,setNodeModal]=useState(false),[lineModal,setLineModal]=useState(false),[ruleModal,setRuleModal]=useState(false),[ruleBatchModal,setRuleBatchModal]=useState(false);
  ...
  const reorderNodes=async(id:string,direction:MoveDirection)=>{const next=moveOrdered(nodes,id,direction);if(next===nodes)return;setNodes(next);if(demo)return;try{await api("/api/v1/nodes/order",{method:"PUT",body:JSON.stringify({ids:next.map(node=>node.id)})})}catch(e){setNodes(nodes);setError((e as Error).message)}};
  const reorderRules=async(rule:Rule,direction:MoveDirection)=>{const lineID=rule.line_id||"",next=moveOrdered(rules,rule.id,direction,item=>(item.line_id||"")===lineID);if(next===rules)return;setRules(next);if(demo)return;try{await api("/api/v1/rules/order",{method:"PUT",body:JSON.stringify({ids:next.map(item=>item.id)})})}catch(e){setRules(rules);setError((e as Error).message)}};
  return <div className="shell" onClickCapture={handleCopyClick}>
    ...
    <main className="main">
      ...
      {view==="rules"&&<Rules
        ...
        onBatch={lineID=>{setPreferredLine(lineID);setRuleBatchModal(true)}}
    ...
    />}
    {ruleBatchModal&&lines.find(line=>line.id===preferredLine)&&<RuleBatchModal line={lines.find(line=>line.id===preferredLine)!} demo={demo} onClose={()=>setRuleBatchModal(false)} onImported={refresh}/>}
              ...
              <div className="card-actions">
                <span className="secondary-actions">
                  <span className="order-actions" aria-label="调整服务器顺序">
                    <button className="icon-button compact" title="上移服务器" aria-label={`上移 ${n.name}`} disabled={index===0} onClick={()=>onMove(n.id,-1)}>↑</button>
                    <button className="icon-button compact" title="下移服务器" aria-label={`下移 ${n.name}`} disabled={index===nodes.length-1} onClick={()=>onMove(n.id,1)}>↓</button>
                  </span>
...
function Lines({nodes,lines,rules,probes,onAdd,onEdit,onRule,onDelete}:{nodes:Node[];lines:Line[];rules:Rule[];probes:LinkProbe[];onAdd:()=>void;onEdit:(line:Line)=>void;onRule:(id:string)=>void;onDelete:(id:string)=>void}){return <section className="table-card"><div className="section-head"><div><p className="eyebrow">延迟探测 · 多出口自动切换</p><h3>线路</h3></div><button className="outline" onClick={onAdd}>＋ 创建线路</button></div><div className="line-grid">{lines.map(line=>{const ingress=nodes.find(n=>n.id===line.ingress_node_id),activeID=line.active_egress_node_id||line.egress_node_id,egress=nodes.find(n=>n.id===activeID),count=rules.filter(r=>r.line_id===line.id).length,probe=probes.find(p=>p.ingress_node_id===line.ingress_node_id&&p.egress_node_id===activeID),exitCount=(line.egress_node_ids?.length||1),probeClass=!probe||!probe.has_succeeded?"unknown":probe.packet_loss>=100?"bad":probe.packet_loss>0?"warn":"good";return <article className="line-card" key={line.id}><div className="line-card-head"><div><span className="line-badge">{modeName(line.mode)}</span>{line.failover_enabled&&<span className="line-badge failover">自动主备</span>}<h3>{line.name}</h3><p>{lineEngineText(line)} · {count} 条转发 · {exitCount} 个出口 · 当前端口池 {linePortRange(line,activeID)||"不限制"}</p></div><i className={egress?.status||"offline"}/></div>{line.mode==="dual_managed"&&<div className={`link-quality ${probeClass}`}><span>当前出口：<b>{egress?.name||"未选择"}</b></span><span>{probe?.has_succeeded?`延迟 ${probe.latency_ms.toFixed(1)} ms · 丢包 ${probe.packet_loss.toFixed(0)}%`:probe?"ICMP 不可用 · 按 Agent 在线状态判断":"等待入口 Agent 探测"}</span></div>}<div className="route-visual"><div><small>{line.mode==="dual_managed"?`公网入口 · ${engineName(line.ingress_engine||line.engine)}`:"已有入口"}</small><b>{line.mode==="dual_managed"?ingress?.name:"已有入口规则"}</b><code>{line.mode==="dual_managed"?(ingress?.public_address||"待配置"):"不由面板管理"}</code></div><span>→</span><div><small>{line.failover_enabled?`当前活动出口 · ${engineName(line.egress_engine||line.engine)}`:`出口接管 · ${engineName(line.egress_engine||line.engine)}`}</small><b>{egress?.name}</b><code>{line.mode==="exit_only"?(line.listen_address||egress?.private_address||"待配置"):(egress?.private_address||"待配置")}</code></div><span>→</span><div><small>规则目标</small><b>落地服务器</b><code>每条转发填写</code></div></div><div className="card-actions"><button className="primary" onClick={()=>onRule(line.id)}>＋ 在此线路新建转发</button><span className="secondary-actions"><button className="outline" onClick={()=>onEdit(line)}>修改线路</button><button className="danger-link" onClick={()=>onDelete(line.id)}>删除</button></span></div></article>})}</div>{!lines.length&&<Empty title="还没有线路" description="把入口、出口和各自的转发引擎组合成一条可复用线路。" action={onAdd}/>}</section>}

function Rules({nodes,lines,rules,traffic,targetProbes,onTraffic,onAdd,onBatch,onEdit,onMove,onToggle,onDelete}:{nodes:Node[];lines:Line[];rules:Rule[];traffic:RuleTraffic[];targetProbes:TargetProbe[];onTraffic:(r:Rule)=>void;onAdd:(id?:string)=>void;onBatch:(id:string)=>void;onEdit:(r:Rule)=>void;onMove:(r:Rule,direction:MoveDirection)=>void;onToggle:(r:Rule)=>void;onDelete:(id:string)=>void}){const groups=lines.map(line=>({line,rules:rules.filter(r=>r.line_id===line.id)}));const legacy=rules.filter(r=>!r.line_id||!lines.some(l=>l.id===r.line_id));return <div className="rule-groups"><div className="section-head standalone"><div><p className="eyebrow">按线路管理端口与落地</p><h3>全部转发</h3></div><button className="outline" onClick={()=>onAdd()}>＋ 新建转发</button></div>{groups.map(({line,rules:items})=><section className="rule-group" key={line.id}><header><div><span className="line-badge">{modeName(line.mode)}</span>{line.failover_enabled&&<span className="line-badge failover">自动主备</span>}<h3>{line.name}</h3><p>{line.mode==="exit_only"?"已有入口":nodeName(nodes,line.ingress_node_id)} → {nodeName(nodes,line.active_egress_node_id||line.egress_node_id)} · {lineEngineText(line)}</p></div><div className="rule-toolbar"><button className="outline" onClick={()=>onBatch(line.id)}>⇧ TXT 批量添加</button><button className="outline" onClick={()=>onAdd(line.id)}>＋ 添加转发</button></div></header>{items.length?<RuleTable rules={items} lines={lines} traffic={traffic} targetProbes={targetProbes} onTraffic={onTraffic} onEdit={onEdit} onMove={onMove} onToggle={onToggle} onDelete={onDelete}/>:<div className="group-empty">这条线路还没有转发规则</div>}</section>)}{legacy.length>0&&<section className="rule-group"><header><div><span className="line-badge muted">旧规则</span><h3>未归入线路</h3><p>升级前创建的规则仍可正常运行</p></div></header><RuleTable rules={legacy} lines={lines} traffic={traffic} targetProbes={targetProbes} onTraffic={onTraffic} onEdit={onEdit} onMove={onMove} onToggle={onToggle} onDelete={onDelete}/></section>}{!rules.length&&!lines.length&&<Empty title="还没有可用线路" description="创建线路后，转发规则将只需填写端口与落地。" action={()=>onAdd()}/>}</div>}
function RuleTable({rules,lines,traffic,targetProbes,onTraffic,onEdit,onMove,onToggle,onDelete}:{rules:Rule[];lines:Line[];traffic:RuleTraffic[];targetProbes:TargetProbe[];onTraffic:(r:Rule)=>void;onEdit:(r:Rule)=>void;onMove:(r:Rule,direction:MoveDirection)=>void;onToggle:(r:Rule)=>void;onDelete:(id:string)=>void}){return <div className="data-table"><div className="table-row table-labels"><span>规则</span><span>监听</span><span>落地</span><span>业务健康</span><span>规则流量</span><span>限速</span><span>状态</span><span/></div>{rules.map((r,index)=>{const t=traffic.find(item=>item.rule_id===r.id);const line=lines.find(item=>item.id===r.line_id);const activeEgress=line?.active_egress_node_id||r.egress_node_id;const p=targetProbes.find(item=>item.rule_id===r.id&&item.node_id===activeEgress)||targetProbes.find(item=>item.rule_id===r.id);const tcpExpected=r.protocol!=="udp";const quality=p?(tcpExpected&&p.tcp_checked?(p.tcp_success?"good":"bad"):(p.success?(p.packet_loss>0?"warn":"good"):(p.has_succeeded?"bad":"warn"))):"";const healthTitle=!p?"等待出口探测":tcpExpected?(p.tcp_checked?(p.tcp_success?`TCP 正常 · ${p.tcp_latency_ms.toFixed(1)} ms`:`TCP ${tcpErrorName(p.tcp_error)}`):"等待新版 Agent 探测 TCP"):(p.success?`ICMP ${p.latency_ms.toFixed(1)} ms`:p.has_succeeded?"ICMP 当前不可达":"ICMP 不可用");const icmpText=p?(p.success?`网络 ${p.latency_ms.toFixed(1)} ms / 丢包 ${p.packet_loss.toFixed(0)}%`:p.has_succeeded?`网络当前不可达 / 丢包 ${p.packet_loss.toFixed(0)}%`:`网络 ICMP 不可用 / 丢包 ${p.packet_loss.toFixed(0)}%`):"出口 Agent 尚未上报";return <div className="table-row" key={r.id}><span><i className={`protocol ${r.protocol}`}>{r.protocol.toUpperCase()}</i><b>{r.name}</b><small>{ruleEngineText(r)}</small></span><span><b>:{r.listen_port}</b><small>{r.mode==="dual_managed"?"公网入口":"内网接入"}</small></span><span><b>{r.target_host}:{r.target_port}</b><small>当前中继端口 {ruleRelayPort(r,activeEgress)}</small></span><span className={`target-quality ${quality}`} title={r.protocol==="both"?"TCP 检查只代表 TCP 业务端口；UDP 仍以实际业务测试为准":undefined}><b>{healthTitle}</b><small>{icmpText}</small><small>{p?`检查于 ${probeCheckedTime(p.checked_at)}（北京时间）`:"通常 10–20 秒内上报"}</small></span><span className="rule-traffic"><b>↓ {bytes(t?.total_download_bytes||0)} · ↑ {bytes(t?.total_upload_bytes||0)}</b><small>实时 ↓ {speed(t?.download_bytes_per_second||0)} · ↑ {speed(t?.upload_bytes_per_second||0)}</small></span><span><b>↓ {r.download_mbps||"∞"} Mbps</b><small>↑ {r.upload_mbps||"∞"} Mbps</small></span><span><button className={`switch ${r.enabled?"on":""}`} aria-label={r.enabled?"暂停规则":"启用规则"} onClick={()=>onToggle(r)}><i/></button><small>{r.enabled?"运行中":"已暂停"}</small></span><span className="row-actions"><button className="icon-button compact" title="上移规则" aria-label={`上移 ${r.name}`} disabled={index===0} onClick={()=>onMove(r,-1)}>↑</button><button className="icon-button compact" title="下移规则" aria-label={`下移 ${r.name}`} disabled={index===rules.length-1} onClick={()=>onMove(r,1)}>↓</button><button className="icon-button" title="修改转发" aria-label="修改转发" onClick={()=>onEdit(r)}>✎</button><button className="icon-button" title="查看流量详情" aria-label="查看流量详情" onClick={()=>onTraffic(r)}>▥</button><button className="icon-button danger" aria-label="删除规则" onClick={()=>onDelete(r.id)}>×</button></span></div>})}</div>}
...
function Settings({demo,onPasswordChanged,onConfigImported}:{demo:boolean;onPasswordChanged:()=>void;onConfigImported:()=>Promise<void>}){
  ...
  async function exportConfig(){
    ...
    try{
      ...
      if(demo){const ids=new Set([...demoLines.flatMap(line=>[line.ingress_node_id,...line.egress_node_ids]),...demoRules.flatMap(rule=>[rule.ingress_node_id,rule.egress_node_id])]);const backup:ConfigBackup={format:"relay-panel-configuration",schema_version:2,exported_at:new Date().toISOString(),required_nodes:demoNodes.filter(node=>ids.has(node.id)).map(({id,name,role})=>({id,name,role})),lines:demoLines,rules:demoRules};blob=new Blob([JSON.stringify(backup,null,2)],{type:"application/json"})}
...
function LineModal({nodes,probes,initial,busy,onClose,onSave}:{nodes:Node[];probes:LinkProbe[];initial:Line|null;busy:boolean;onClose:()=>void;onSave:(l:typeof blankLine)=>void}){
  ...
  const initialPortRanges=Object.fromEntries(initialEgresses.map(id=>[id,Object.prototype.hasOwnProperty.call(initial?.egress_port_ranges||{},id)?initial?.egress_port_ranges?.[id]||"":initial?.relay_port_range||""]));
  ...
  }:{...blankLine,ingress_node_id:firstIngress?.id||"",egress_node_id:firstEgress?.id||"",egress_node_ids:firstEgress?[firstEgress.id]:[],active_egress_node_id:firstEgress?.id||"",relay_port_range:firstEgress?.default_relay_port_range||"",egress_port_ranges:firstEgress?{[firstEgress.id]:firstEgress.default_relay_port_range||""}:{}});
  ...
  const setPrimary=(id:string)=>setLine(current=>{
    ...
    const ranges={...current.egress_port_ranges,[id]:current.egress_port_ranges[id]??node?.default_relay_port_range??""};
    return {...current,egress_node_id:id,egress_node_ids:current.mode==="exit_only"?[id]:ids,active_egress_node_id:ids.includes(current.active_egress_node_id)?current.active_egress_node_id:id,ingress_node_id:current.mode==="exit_only"?id:current.ingress_node_id,listen_address:current.mode==="exit_only"?(node?.private_address||""):"0.0.0.0",relay_port_range:ranges[id],egress_port_ranges:ranges};
  });
  const toggleBackup=(id:string)=>setLine(current=>{
    ...
    return {...current,egress_node_ids:ordered,active_egress_node_id:ordered.includes(current.active_egress_node_id)?current.active_egress_node_id:current.egress_node_id,failover_enabled:ordered.length>1?current.failover_enabled:false,egress_port_ranges:selected?current.egress_port_ranges:{...current.egress_port_ranges,[id]:current.egress_port_ranges[id]??node?.default_relay_port_range??""}};
  });
  const setEgressRange=(id:string,value:string)=>setLine(current=>({...current,egress_port_ranges:{...current.egress_port_ranges,[id]:value},relay_port_range:id===current.egress_node_id?value:current.relay_port_range}));
  ...
  return <Modal title={initial?"修改线路":"创建线路"} kicker={initial?"调整拓扑":"线路拓扑"} onClose={onClose}><form className="form-grid" onSubmit={e=>{e.preventDefault();const ids=line.mode==="exit_only"?[line.egress_node_id]:orderedEgresses;onSave({...line,relay_port_range:line.egress_port_ranges[line.egress_node_id]??line.relay_port_range,engine:line.egress_engine,ingress_engine:line.mode==="exit_only"?line.egress_engine:line.ingress_engine,ingress_node_id:line.mode==="exit_only"?line.egress_node_id:line.ingress_node_id,egress_node_ids:ids,active_egress_node_id:ids.includes(line.active_egress_node_id)?line.active_egress_node_id:line.egress_node_id,failover_enabled:line.mode==="dual_managed"&&line.failover_enabled&&ids.length>1})}}>
    ...
    <div className="egress-port-ranges full"><div><b>{line.mode==="dual_managed"?"各出口可用中继端口":"出口可用接入端口"}</b><small>每个 NAT 出口可填写服务商分配的不同端口或范围；留空表示该出口不限制。</small></div>{orderedEgresses.map((id,index)=>{const node=nodes.find(n=>n.id===id);return <label key={id}>{index===0?"主出口":"备用出口"} · {node?.name||id}<input value={line.egress_port_ranges[id]??(id===line.egress_node_id?line.relay_port_range:"")} onChange={e=>setEgressRange(id,e.target.value)} placeholder="例如 1301-1349,25000"/><small>{node?.private_address||"内网 IP 待配置"}</small></label>})}</div>
    ...
    <div className="route-summary full"><small>{initial?"修改后的路径":"保存后的路径"}</small><b>{line.mode==="dual_managed"?(ingress?.name||"入口服务器"):"已有入口"} → {primary?.name||"主出口"}{orderedEgresses.length>1?`（另有 ${orderedEgresses.length-1} 个备用）`:""} → 每条规则的落地地址</b><span>{line.mode==="dual_managed"?`入口 ${engineName(line.ingress_engine)} → 出口 ${engineName(line.egress_engine)}`:`出口 ${engineName(line.egress_engine)}`} · {orderedEgresses.length} 个出口独立分配中继端口</span></div>
...
function RuleBatchModal({line,demo,onClose,onImported}:{line:Line;demo:boolean;onClose:()=>void;onImported:()=>Promise<void>}){
  const example=`# 每行：入口端口 落地IP 落地端口 [协议] [名称]\n11301 38.49.57.74 36666\n11302 38.49.57.75 36667 tcp 业务二`;
  const [text,setText]=useState(""),[busy,setBusy]=useState(false),[notice,setNotice]=useState(""),[error,setError]=useState("");
  const fileInput=useRef<HTMLInputElement>(null);
  const downloadTemplate=()=>{
    const blob=new Blob([example+"\n"],{type:"text/plain;charset=utf-8"}),url=URL.createObjectURL(blob),anchor=document.createElement("a");
    anchor.href=url;anchor.download=`relay-rules-${line.name.replace(/[^\w\u4e00-\u9fff-]+/g,"-")}.txt`;document.body.appendChild(anchor);anchor.click();anchor.remove();URL.revokeObjectURL(url);setNotice("已下载 TXT 模板，按示例每行填写一条规则。");
  };
  const readFile=async(file:File)=>{
    setError("");setNotice("");
    try{if(file.size>1024*1024)throw new Error("批量规则文件不能超过 1 MB");setText(await file.text());setNotice(`已读取 ${file.name}，请确认内容后导入。`)}catch(err){setError((err as Error).message)}finally{if(fileInput.current)fileInput.current.value=""}
  };
  const request=async(dryRun:boolean)=>{
    const query=new URLSearchParams({line_id:line.id});if(dryRun)query.set("dry_run","1");
    const response=await fetch(`/api/v1/rules/import-text?${query}`,{method:"POST",credentials:"include",cache:"no-store",headers:{"Content-Type":"text/plain;charset=utf-8"},body:text});
    if(!response.ok){let message=`请求失败 (${response.status})`;try{message=(await response.json()).error||message}catch{/* non-JSON */}throw new Error(message)}
