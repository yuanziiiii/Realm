...
type Line = { id:string; name:string; mode:Mode; ingress_node_id:string; egress_node_id:string; egress_node_ids:string[]; active_egress_node_id:string; failover_enabled:boolean; listen_address:string; relay_port_range:string; engine:Engine; enabled:boolean };
...
const demoLines:Line[] = [
  {id:"line-gz-hk",name:"广州 → 香港专线",mode:"dual_managed",ingress_node_id:"in-gz",egress_node_id:"out-hkg",egress_node_ids:["out-hkg","out-sg"],active_egress_node_id:"out-hkg",failover_enabled:true,listen_address:"0.0.0.0",relay_port_range:"",engine:"nftables",enabled:true},
  {id:"line-hk-exit",name:"香港出口接管",mode:"exit_only",ingress_node_id:"out-hkg",egress_node_id:"out-hkg",egress_node_ids:["out-hkg"],active_egress_node_id:"out-hkg",failover_enabled:false,listen_address:"10.24.0.3",relay_port_range:"30000-39999",engine:"realm",enabled:true},
];
...
const blankLine={name:"",mode:"dual_managed" as Mode,ingress_node_id:"",egress_node_id:"",egress_node_ids:[] as string[],active_egress_node_id:"",failover_enabled:false,listen_address:"",relay_port_range:"",engine:"nftables" as Engine,enabled:true};
...
function LineModal({nodes,probes,initial,busy,onClose,onSave}:{nodes:Node[];probes:LinkProbe[];initial:Line|null;busy:boolean;onClose:()=>void;onSave:(l:typeof blankLine)=>void}){
  ...
  const [line,setLine]=useState(()=>initial?{
    ...
    engine:initial.engine,enabled:initial.enabled,
  ...
  return <Modal title={initial?"修改线路":"创建线路"} kicker={initial?"调整拓扑":"线路拓扑"} onClose={onClose}><form className="form-grid" onSubmit={e=>{e.preventDefault();const ids=line.mode==="exit_only"?[line.egress_node_id]:orderedEgresses;onSave({...line,ingress_node_id:line.mode==="exit_only"?line.egress_node_id:line.ingress_node_id,egress_node_ids:ids,active_egress_node_id:ids.includes(line.active_egress_node_id)?line.active_egress_node_id:line.egress_node_id,failover_enabled:line.mode==="dual_managed"&&line.failover_enabled&&ids.length>1})}}>
    ...
    <label className="full">线路引擎<select value={line.engine} onChange={field("engine")}><option value="nftables">nftables（内核转发）</option><option value="realm">Realm</option></select></label>
    {initial&&<div className="change-warning full">保存后，线路下已有转发会迁移到新的入口和出口，并自动重新下发。</div>}
    <div className="route-summary full"><small>{initial?"修改后的路径":"保存后的路径"}</small><b>{line.mode==="dual_managed"?(ingress?.name||"入口服务器"):"已有入口"} → {primary?.name||"主出口"}{orderedEgresses.length>1?`（另有 ${orderedEgresses.length-1} 个备用）`:""} → 每条规则的落地地址</b><span>出口端口：{line.relay_port_range||"不限制（自动从 30000 起分配）"}</span></div>
