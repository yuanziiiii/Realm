...
type RuleBatchItem = { name?:string; protocol?:"tcp"|"udp"|"both"; listen_port:number; target_host:string; target_port?:number; upload_mbps?:number; download_mbps?:number; burst_kbytes?:number; enabled?:boolean };
type RuleBatchFile = { format?:string; schema_version?:number; line_id?:string; rules:RuleBatchItem[] };
  ...
  return <div className="shell" onClickCapture={handleCopyClick}>
    ...
    <main className="main">
      ...
      {view==="rules"&&<Rules
        ...
        onBatch={()=>{setPreferredLine(lines[0]?.id||"");setRuleBatchModal(true)}}
    ...
    />}
    {ruleBatchModal&&<RuleBatchModal lines={lines} initialLine={preferredLine} demo={demo} onClose={()=>setRuleBatchModal(false)} onImported={refresh}/>}
...
function Rules({nodes,lines,rules,traffic,targetProbes,onTraffic,onAdd,onBatch,onEdit,onToggle,onDelete}:{nodes:Node[];lines:Line[];rules:Rule[];traffic:RuleTraffic[];targetProbes:TargetProbe[];onTraffic:(r:Rule)=>void;onAdd:(id?:string)=>void;onBatch:()=>void;onEdit:(r:Rule)=>void;onToggle:(r:Rule)=>void;onDelete:(id:string)=>void}){const groups=lines.map(line=>({line,rules:rules.filter(r=>r.line_id===line.id)}));const legacy=rules.filter(r=>!r.line_id||!lines.some(l=>l.id===r.line_id));return <div className="rule-groups"><div className="section-head standalone"><div><p className="eyebrow">先选线路，再填端口与落地</p><h3>全部转发</h3></div><div className="rule-toolbar"><button className="outline" onClick={onBatch} disabled={!lines.length}>⇧ 批量导入</button><button className="outline" onClick={()=>onAdd()}>＋ 新建转发</button></div></div>{groups.map(({line,rules:items})=><section className="rule-group" key={line.id}><header><div><span className="line-badge">{modeName(line.mode)}</span>{line.failover_enabled&&<span className="line-badge failover">自动主备</span>}<h3>{line.name}</h3><p>{line.mode==="exit_only"?"已有入口":nodeName(nodes,line.ingress_node_id)} → {nodeName(nodes,line.active_egress_node_id||line.egress_node_id)} · {lineEngineText(line)}</p></div><button className="outline" onClick={()=>onAdd(line.id)}>＋ 添加转发</button></header>{items.length?<RuleTable rules={items} lines={lines} traffic={traffic} targetProbes={targetProbes} onTraffic={onTraffic} onEdit={onEdit} onToggle={onToggle} onDelete={onDelete}/>:<div className="group-empty">这条线路还没有转发规则</div>}</section>)}{legacy.length>0&&<section className="rule-group"><header><div><span className="line-badge muted">旧规则</span><h3>未归入线路</h3><p>升级前创建的规则仍可正常运行</p></div></header><RuleTable rules={legacy} lines={lines} traffic={traffic} targetProbes={targetProbes} onTraffic={onTraffic} onEdit={onEdit} onToggle={onToggle} onDelete={onDelete}/></section>}{!rules.length&&!lines.length&&<Empty title="还没有可用线路" description="创建线路后，转发规则将只需填写端口与落地。" action={()=>onAdd()}/>}</div>}
...
function RuleBatchModal({lines,initialLine,demo,onClose,onImported}:{lines:Line[];initialLine:string;demo:boolean;onClose:()=>void;onImported:()=>Promise<void>}){
  const [lineID,setLineID]=useState(initialLine||lines[0]?.id||""),[busy,setBusy]=useState(false),[notice,setNotice]=useState(""),[error,setError]=useState("");
  const fileInput=useRef<HTMLInputElement>(null);
  const line=lines.find(item=>item.id===lineID);
  const downloadTemplate=()=>{
    const template:RuleBatchFile={format:"relay-panel-rule-batch",schema_version:1,line_id:lineID,rules:[
      {name:"业务一",protocol:"both",listen_port:11301,target_host:"38.49.57.74",target_port:36666,upload_mbps:0,download_mbps:0,burst_kbytes:512,enabled:true},
      {name:"业务二",protocol:"tcp",listen_port:11302,target_host:"38.49.57.75",target_port:36667,upload_mbps:0,download_mbps:100,burst_kbytes:512,enabled:true},
    ]};
    const blob=new Blob([JSON.stringify(template,null,2)],{type:"application/json"}),url=URL.createObjectURL(blob),anchor=document.createElement("a");
    anchor.href=url;anchor.download=`relay-rules-${(line?.name||"template").replace(/[^\w\u4e00-\u9fff-]+/g,"-")}.json`;document.body.appendChild(anchor);anchor.click();anchor.remove();URL.revokeObjectURL(url);setNotice("已下载批量规则模板，按示例修改后再上传。");
  };
  const importFile=async(file:File)=>{
    setBusy(true);setError("");setNotice("");
    try{
      if(!lineID)throw new Error("请先选择导入线路");
      if(file.size>1024*1024)throw new Error("批量规则文件不能超过 1 MB");
      const raw:unknown=JSON.parse(await file.text());
      const parsed:RuleBatchFile=Array.isArray(raw)?{rules:raw as RuleBatchItem[]}:(raw as RuleBatchFile);
      if(!parsed||!Array.isArray(parsed.rules))throw new Error("JSON 中缺少 rules 数组");
      const batch:RuleBatchFile={...parsed,format:"relay-panel-rule-batch",schema_version:1,line_id:lineID};
      if(demo){setNotice(`预览模式已读取 ${batch.rules.length} 条规则，不会写入。`);return}
      const body=JSON.stringify(batch);
      const preview=await api<RuleBatchResult>("/api/v1/rules/import?dry_run=1",{method:"POST",body});
      if(!window.confirm(`预检通过：将在线路“${line?.name||lineID}”中创建 ${preview.rules} 条规则。是否继续？`)){setNotice("已取消导入，当前规则没有变化。");return}
      const result=await api<RuleBatchResult>("/api/v1/rules/import",{method:"POST",body});
      await onImported();setNotice(`导入完成：已创建 ${result.rules} 条规则并统一下发。`);
    }catch(err){setError(err instanceof SyntaxError?"文件不是有效的 JSON":(err as Error).message)}finally{setBusy(false);if(fileInput.current)fileInput.current.value=""}
  };
  return <Modal title="批量导入转发" kicker="JSON 批量创建" onClose={onClose}><div className="batch-import">
    <label>导入到线路<select value={lineID} onChange={event=>{setLineID(event.target.value);setNotice("");setError("")}}>{lines.map(item=><option key={item.id} value={item.id}>{item.name} · {modeName(item.mode)} · {lineEngineText(item)}</option>)}</select></label>
    {line&&<div className="selected-line"><span className="line-badge">{modeName(line.mode)}</span><div><b>{line.name}</b><small>每条 JSON 规则只需填写入口端口、落地 IP 和落地端口；中继端口由面板自动分配。</small></div><em>{line.egress_node_ids?.length||1} 个出口</em></div>}
    <div className="batch-guide"><b>推荐先下载模板</b><p>保留 JSON 字段结构，可一次放入最多 500 条规则。上传时会整批检查入口端口、各出口 NAT 端口池和现有规则冲突。</p><code>{`{"name":"业务一","protocol":"both","listen_port":11301,"target_host":"38.49.57.74","target_port":36666}`}</code></div>
    {notice&&<div className="success-notice" role="status">{notice}</div>}{error&&<div className="form-error" role="alert">{error}</div>}
    <div className="modal-actions"><button type="button" className="outline" disabled={busy||!lineID} onClick={downloadTemplate}>下载 JSON 模板</button><button type="button" className="primary" disabled={busy||!lineID} onClick={()=>fileInput.current?.click()}>{busy?"校验中…":"选择 JSON 并导入"}</button><input ref={fileInput} className="config-input" type="file" accept=".json,application/json" onChange={event=>{const file=event.target.files?.[0];if(file)void importFile(file)}}/></div>
  </div></Modal>;
}
